...
- (void)layoutSubviews
{
    ...
    self.statusLabel.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y-20, self.bounds.size.width, self.bounds.size.height-20);//self.bounds;
}
...
//          NSLog(@"bound=%f,%f",self.frame.size.width,self.frame.size.height);//
          self.transform = CGAffineTransformMakeRotation(-M_PI/2);
          self.mj_x = 0;
        
//  //      self.frame = CGRectMake(0, 0, self.frame.size.height, self.frame.size.width);
//           NSLog(@"1111self.frame=%f,%f;%f,%f",self.frame.origin.x,self.frame.origin.y,self.frame.size.width,self.frame.size.height);//
...
- (void)adjustFrameWithContentSize
{
    ...
    // 表格的高度
    CGFloat scrollHeight = self.scrollView.mj_width - self.scrollViewOriginalInset.top - self.scrollViewOriginalInset.bottom;
    // 设置位置和尺寸
    self.mj_y = MAX(contentHeight, scrollHeight);
  NSLog(@"contentHeight=%f,scrollHeight=%f",contentHeight ,scrollHeight);
       // 内容的宽度
//        CGFloat contentWidth = self.scrollView.mj_contentSizeHeight;
//        // 表格的高度
//        CGFloat scrollHWidth = self.scrollView.mj_width - self.scrollViewOriginalInset.left - self.scrollViewOriginalInset.right;
//        // 设置位置和尺寸
//        self.mj_y = MAX(contentWidth, scrollHWidth)-self.mj_width;
//        NSLog(@"contentWidth = %f,scrollHWidth=%f,self.mj_x=%f",contentWidth,scrollHWidth,self.mj_x);
...
{
    CGFloat h = self.scrollView.frame.size.width - self.scrollViewOriginalInset.bottom - self.scrollViewOriginalInset.top;
