...
- (void)setupViews
{
    ...
    */
    ...
    for (int i = 0; i < imageItems.count; i++) {
        ...
        imageView.tag = i;
        //添加点击事件
        UITapGestureRecognizer *singleTap = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickPageImage:)] autorelease];
...
-(void)clickPageImage:(UITapGestureRecognizer *)sender {
    NSLog(@"click button tag is %d",sender.view.tag);
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(foucusImageFrame:didSelectItem:)]) {
        [self.delegate foucusImageFrame:self didSelectItem:(SGFocusImageItem *) sender.view];
    }
}
