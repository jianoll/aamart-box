...
- (void)viewDidLoad {
    ...
    [[AFAppDotNetAPIClient sharedClient] getUserIntegral:^(NSArray *repondData, NSError *error) {
        if(!error)
        {
            if (repondData != nil) {
            }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"PersonlInfo"]) {
        ...
        //PersonalInfoTableViewController *info = [mainStoryboard instantiateViewControllerWithIdentifier:@"PersonalInfoTableViewController"];
        
        PersonalInfoTableViewController *info = segue.destinationViewController;
        
        info.personInfo = self.userInfo[0];
    }
    else if ([segue.identifier isEqualToString:@"carInfo"]){
        NSLog(@"carInfo");
        
        PersonalInfoTableViewController *info = segue.destinationViewController;
        
        info.personInfo = self.userInfo[1];
    }
}
