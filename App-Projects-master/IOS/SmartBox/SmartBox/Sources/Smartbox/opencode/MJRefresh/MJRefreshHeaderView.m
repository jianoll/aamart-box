...
{
    // 当前的contentOffset
    CGFloat currentOffsetX = self.scrollView.mj_contentOffsetX;
    // 头部控件刚好出现的offsetY
    ...
    if (self.scrollView.isDragging) {
        // 普通 和 即将刷新 的临界点
        CGFloat normal2pullingOffsetX = happenOffsetX-self.mj_width;
        NSLog(@"normal2pullingOffsetX = %f, currentOffsetX =%f", normal2pullingOffsetX,currentOffsetX);
        if (self.state == MJRefreshStateNormal && currentOffsetX < normal2pullingOffsetX) {
            // 转为即将刷新状态
...
#warning 这句代码修复了，top值不断累加的bug
                    self.scrollView.mj_contentInsetLeft -= self.mj_width;
                }];
		...
		case MJRefreshStateRefreshing: // 正在刷新中
        {
            ...
            [UIView animateWithDuration:MJRefreshFastAnimationDuration animations:^{
                // 1.增加滚动区域
                CGFloat left = self.scrollViewOriginalInset.left +self.mj_width;
                self.scrollView.mj_contentInsetLeft = left;
                
                // 2.设置滚动位置
//                self.scrollView.mj_contentOffsetX = - left;
            }];
			break;
        }
