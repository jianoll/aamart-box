...
- (void)addCellWithInfo:(NSObject<DTGridViewCellInfoProtocol> *)info {
    ...
    NSLog(@"info = [%f,%f]",info.frame.size.width,info.frame.size.height);
...
//    NSLog(@"visibleLeft =[%f],visibleRight=[%f], infoLeft=[%f],infoRight=[%f]",visibleLeft, visibleRight,infoLeft,infoRight);
...
- (CGFloat)CellWidthForRow{
    static CGFloat width =  0;
    if(width == 0)
    {
        width =[self.dataSource gridView:self widthForCellAtRow:0 column:0];
    }
    return width;
}
...
- (void)checkViews {
    ...
    NSLog(@"cellInfoForCellsOnScreen COUNT = %d",[cellInfoForCellsOnScreen count]);
	if ([cellInfoForCellsOnScreen count] == 0) {
		[self initialiseViews];
        self.contentOffset = CGPointMake(self.contentSize.width - self.frame.size.width, 0);
		return;
...
//			[c retain];
            DTGridViewCell* returnC = c;
			[freeCells removeObject:c];
			[returnC prepareForReuse];
            return returnC;//[c autorelease];
		}
	}
...
//	if (self.pagingEnabled)
        ...
        {
            [self.delegate pagedGridView:self didScrollToRow:((NSInteger)(self.contentOffset.y / self.frame.size.height)) column:((NSInteger)((self.contentOffset.x + [self CellWidthForRow]/2) / [self CellWidthForRow]))];//bobo nodify +self.frame.size.width/2
            NSLog(@"fireEdgeScroll pagingEnabled");
        }
