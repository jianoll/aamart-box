...
- (void)viewDidLoad {
    ...
    _txtUserNameView.text = [theUserDefaults objectForKey:LastUserID];//
    _txtPwdView.text = [theUserDefaults objectForKey:LastUserPassword];
    
//#ifdef debugbobo
//#ifdef test
//    _txtUserNameView.text = @"689533";//
//    _txtPwdView.text = @"158376";
//#else
//    _txtUserNameView.text = @"949638";//
//    _txtPwdView.text = @"418002";
//#endif
//#endif
    isSavePwd = YES;
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:LoginSuccessed_Notification object:nil userInfo:nil];//delete by bobo
        //获取用户信息
        [[AFAppDotNetAPIClient sharedClient] getUserDetailInfo:^(NSArray *repondData, NSError *error) {
            if(repondData.count>=2)
            {
                NSDictionary* UserDetailDic = repondData[1];
                [LoginManager sharedInstance].nick   = [UserDetailDic objectForKey:@"nickName"];
                [LoginManager sharedInstance].sex    = [[UserDetailDic objectForKey:@"sex"] integerValue];
                
                NSMutableDictionary*  DetailDic = [NSMutableDictionary dictionaryWithDictionary:repondData[0]];
                [DetailDic addEntriesFromDictionary:UserDetailDic];
                //写入数据库
               [[UserDAO sharedManager] updateUserInfo:[user.userId integerValue] detailinfo:DetailDic bCreate:YES];
#ifdef debugbobo
                UserInfo_assist* debug_assit = [ [UserDAO sharedManager] find:[user.userId integerValue]];
                NSLog(@"debug_assit=%@",debug_assit);
#endif
            }
