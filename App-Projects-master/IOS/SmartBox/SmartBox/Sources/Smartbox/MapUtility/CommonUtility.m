...
#define ANNOTATION_REGION_PAD_FACTOR 1.22
...
+ (void)zoomMapViewToFitAnnotations:(MAMapView *)mapView animated:(BOOL)animated
{
    ...
    MACoordinateRegion region = MACoordinateRegionForMapRect(mapRect);
     //add by bobo
    CLLocationDegrees offlatitude = region.span.latitudeDelta*(ANNOTATION_REGION_PAD_FACTOR-1)/2;
    //add padding so pins aren't scrunched on the edges
    region.span.latitudeDelta  *= ANNOTATION_REGION_PAD_FACTOR;
    region.span.longitudeDelta *= ANNOTATION_REGION_PAD_FACTOR;
    
//    //bobo
    region.center.latitude += offlatitude;
    ...
    NSLog(@"111111[%f,%f,%f,%f]",mapView.frame.origin.x, mapView.frame.origin.y, mapView.frame.size.width,mapView.frame.size.height);
    [mapView.subviews enumerateObjectsUsingBlock:^(UIView* obj, NSUInteger idx, BOOL *stop) {
