...
//		}
        ...
        UserInfo_assist* loginUser = [[UserInfo_assist alloc]init];
        loginUser.userId = [NSNumber numberWithInt:self.tempUserID];
//        loginUser.emailAccount = tempEmailString;
        loginUser.lastlogindate = loginResponse.syncTime;
        loginUser.pwd = self.tempPassword;
        
        //bobo
        NSDictionary* detailDic = [NSDictionary dictionaryWithObjects:@[loginUser.userId, loginUser.lastlogindate,loginUser.pwd]
                                                                      forKeys:@[@"userId",@"lastlogindate",@"pwd"]];
        [[UserDAO sharedManager] updateUserInfo:self.tempUserID detailinfo:detailDic bCreate:YES];
//        [[UserDAO sharedManager] modifyUserData:loginUser];
		//更改应用配置信息
		NSUserDefaults *saveDefaults = [NSUserDefaults standardUserDefaults];
		[saveDefaults setObject:loginUser.userId forKey:LastUserID];
		[saveDefaults setObject:loginUser.pwd forKey:LastUserPassword];
